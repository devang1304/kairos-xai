...
def _summarise_edges(aggregate: Optional[List[Dict[str, object]]], node_map: Dict[int, str]) -> List[EdgeSummary]:
    ...
    for entry in aggregate:
        ...
        if src_id is None or dst_id is None:
            continue
        src = _resolve_node(int(src_id), node_map) if isinstance(src_id, (int, float)) else str(src_id)
        dst = _resolve_node(int(dst_id), node_map) if isinstance(dst_id, (int, float)) else str(dst_id)
    ...
    for node in nodes:
        ...
        for gnn_event in node.get("gnn", []):
            for edge in gnn_event.get("top_edges", []):
                ...
                src_id = edge.get("src")
                dst_id = edge.get("dst")
                src_label = _resolve_node(int(src_id), node_map) if isinstance(src_id, (int, float)) else "Unknown"
                dst_label = _resolve_node(int(dst_id), node_map) if isinstance(dst_id, (int, float)) else "Unknown"
                gnn_edges.append(
                    {
                        "src": src_label,
                        "dst": dst_label,
...
def _call_gpt_section(
    ...
    system_prompt = (
        "You are an incident-response analyst reviewing Kairos temporal graph explanations. "
...
def _build_gpt_analysis(summary_payload: Dict[str, object]) -> Optional[str]:
    ...
    sections = [
        (
            "What happened?",
            "Summarise the overall activity and whether Kairos detections align with any ground-truth attacks noted in the payload. "
            "Mention the attack goal if it is obvious.",
            "Return one Markdown paragraph containing 2-3 sentences, each no longer than 20-25 words.",
        ),
        (
            "Who's involved?",
            "List the top suspicious nodes or processes with human-readable names, their role/evidence, average loss (avg_loss), "
            "peak loss for any single event (max_event_loss), total loss (total_loss), event counts (event_count), whether any event meets or exceeds the threshold (above_threshold), "
            "and whether the model marks them as malicious.",
            "Return a Markdown table with columns | Node | Role/Evidence | Avg Loss | Peak Loss | Events | Total Loss | >= Threshold | Model Alignment |. "
            "Use 'Yes' or 'No' in the >= Threshold column. Limit to 5 rows.",
        ),
        (
            "Why flagged?",
            "Highlight key relations or events (include timestamps when available) that triggered the alert, using readable node names.",
        ...
        (
            "What next?",
            "Provide concrete follow-up actions for analysts or model owners grounded strictly in the payload.",
            "Return 2-3 Markdown bullet points starting with '- '.",
        ),
    ]
...
def _render_markdown(context: Dict[str, object]) -> str:
    ...
    if high_loss_used > 0:
        ...
        lines.append(
            f"**High-loss events analysed**: none (0 of {high_loss_candidates}; GraphMask runs: {graphmask_events})  "
        )
    ...
    lines.append("| Node | Avg Loss | Peak Loss | Events | Total Loss | >= Threshold |")
    lines.append("| --- | ---: | ---: | ---: | ---: | --- |")
    for node in context["nodes"][:8]:
        ...
        lines.append(
            f"| {node['label']} | {node['score']:.3f} | {peak_loss:.3f} | {node.get('event_count', 0)} | {total_loss:,.3f} | {meets_threshold} |"
        )
    lines.append("")

    lines.append("## Key Graph Insights (GraphMask)")
    lines.append("| Relation | Source | Destination | Weight | Count | Rank Score | Example Times |")
    lines.append("| --- | --- | --- | ---: | ---: | ---: | --- |")
    for edge in context["graph_edges"][:10]:
        times = ", ".join(edge.timestamps) if edge.timestamps else "—"
        lines.append(
            f"| {edge.relation} | {edge.src} | {edge.dst} | {edge.weight:.3f} | {edge.count} | {edge.rank_score:.3f} | {times} |"
        )
    lines.append("")
    ...
    for node in context["nodes"][:8]:
        ...
        peak_loss = node.get("max_event_loss", 0.0)
        lines.append(
            f"### {node['label']} (avg_loss={node['score']:.3f}, peak_loss={peak_loss:.3f}, events={event_count}, total_loss={total_loss:,.3f})"
        )
        if node["gnn_edges"]:
            top = ", ".join(
                f"{edge['relation']} {edge['src']} → {edge['dst']} ({edge['weight']:.3f}, {edge.get('when', '—')})"
...
def build_reports(
    ...
    aggregate_edges = aggregate_edges_raw if isinstance(aggregate_edges_raw, list) else []
    ...
    threshold_value = float(threshold_raw) if threshold_raw is not None else 0.0
    for node in node_entries:
        node["above_threshold"] = node.get("max_event_loss", 0.0) >= threshold_value
    ...
    high_loss_candidates = int(metrics.get("high_loss_candidates", inferred_default)) if metrics else inferred_default
    high_loss_used = int(metrics.get("high_loss_used", inferred_default)) if metrics else 0
    graphmask_events = int(metrics.get("graphmask_events", inferred_default)) if metrics else inferred_default
    ...
    if not metrics_provided and graphmask_events:
        ...
        metrics_note = (
            ...
            else f"GraphMask statistics inferred from payload; large event count ({graphmask_events}) may indicate fallback values."
        )
